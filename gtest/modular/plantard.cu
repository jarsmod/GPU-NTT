...
TEST(GpuModularPlantard, modOneIsZero) {
    
    ASSERT_EQ( 0 ,0);
    
}

TEST(GpuModularPlantard, singleModAdd) {
    /**
     * input should be 0...q-1
     >>> ((1<<16)+(1<<16)-2) % (1<<16)
    65534
    */
    using namespace plantard64_gpu;
    
    Data a = (1 << 16) - 1 , b = (1 << 16) - 1;
    Modulus q(1 << 16);
    PlantardOperations pred;
    std::cout <<"pred.add(a, b, q) " << pred.add(a, b, q) << std::endl;
    ASSERT_EQ( pred.add(a, b, q) , 65534);
    
}

TEST(GpuModularPlantard, multiplemodAddTest) {
    /**
     * input should be 0...q-1
     ...
     */
    using namespace plantard64_gpu; //TODO: change them to _gpu
...
TEST(GpuModularPlantard, multiplemodSubTest) {
    //TODO
}
#endif